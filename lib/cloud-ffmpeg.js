...
class CloudFFmpeg {
  ...
  run(data) {
    ...
    const outputWithLocalPath = _.zip(
      data.output,
    ...
    return new Promise((resolve, reject) => {
      command.on('end', (stdout, stderr) => {
        const uploadTasks = outputWithLocalPath.map((sourceWithLocalPath) => {
        ...
        const removeAfterUploading = uploadTasks.map(() => {
          new Promise((removeResolve) => {
            fs.unlink(localPath, removeResolve);
          });
        });
        Promise.all(removeAfterUploading).then(resolve).catch(reject);
      }).run();
    });
  }
}
